...
if __name__ == "__main__":
    ...
    ls = os.listdir(path='postProcessing/samplenew/')
    ls.sort()
    m = 0
    for i in ls:
        print(i)

        if os.path.isfile('postProcessing/samplenew/'+i):
            continue

        if float(i) < 49 or float(i) > 56:
            continue

        dirName = 'postProcessing/samplenew/' + i + '/data_U.xy'

        if float(i) >= 10:
            dirName = 'postProcessing/samplenew/' + i + '/data_U_vorticity.xy'
        ...
        while True:
            ...
            if numberold !=numbers[0] and numbers[0] >= 0.96  and numbers[1] <= 0.3:
    ...
    uls4 = np.load('uls2.npy')
 
    uls3 = np.reshape(uls3, (m, -1))
    uls3 = np.concatenate((uls4[:2400] , uls3 ), axis=0)
    np.save('uls2', uls3)
