...
def SPOD(X, nDFT, nBlks, dT):
    ...
    window = hamming(nDFT).reshape(nDFT, 1)
    ...
    P = np.zeros((nFreq, nx, nBlks), dtype=np.csingle)

    for i in range(nFreq):

        Q_hat_f = Q_hat[i, :, :]
        C = dot(Q_hat_f.conj().T, Q_hat_f) / nBlks
        Lambda, Theta = eig(C)
        index = np.argsort(Lambda)[::-1]
        Lambda = Lambda[index]
        Theta = Theta[:, index]
